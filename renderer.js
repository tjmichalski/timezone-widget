  ...
  if (window.electronAPI) {
    ...
    if (Array.isArray(saved) && saved.length) {
      activeClocks = saved.map(c => ({
        ...c,
        businessHours: c.businessHours ?? { start: 9, end: 18 },
      }));
    }
...
function addClock(iana, defaultLabel) {
  ...
  activeClocks.push({ id, iana, customLabel: defaultLabel, businessHours: { start: 9, end: 18 } });
  persist();
  renderAllClocks();
}
...
function updateBusinessHours(id, start, end) {
  const clock = activeClocks.find(c => c.id === id);
  if (clock) { clock.businessHours = { start, end }; persist(); tick(); }
}
...
function buildCard(clock) {
  const bh = clock.businessHours ?? { start: 9, end: 18 };
  ...
  editSvg.setAttribute('class', 'edit-icon');
  editSvg.setAttribute('viewBox', '0 0 24 24');
  editSvg.setAttribute('fill', 'currentColor');
  editSvg.innerHTML = '<path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1 1 0 0 0 0-1.41l-2.34-2.34a1 1 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>';

  labelWrap.appendChild(labelInput);
  labelWrap.appendChild(editSvg);
  ...
  const toHHMM = h => `${String(h).padStart(2, '0')}:00`;
  ...
  card.addEventListener('dragend', () => {
    ...
    clocksContainer.querySelectorAll('.drag-over').forEach(c => c.classList.remove('drag-over'));
  });
...
function tick() {
  ...
  clocksContainer.querySelectorAll('.clock-card').forEach(card => {
    ...
    if (clock) updateCard(card, clock, now);
  });
  updateFooterDate(now);
}

function updateCard(card, clock, now = new Date()) {
  ...
  try {
    ...
    if (dayDiffEl) {
      const diff = getDayDiff(clock.iana, now);
